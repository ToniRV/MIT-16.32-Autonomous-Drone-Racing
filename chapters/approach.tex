...
The attitude $\boldsymbol{\eta}$ of the drone with respect to the world frame is given using Euler angles: pitch $\theta$, roll $\phi$, and yaw $\psi$.
...
\begin{itemize}
  \item Generic input constraints can be formulated as: 
  $$M_{i}^{-} \leq u_{i}(t) \leq M_{i}^{+},$$
  where $M_{i}^{-}$ is the lower bound of control input $u_{i}(t)$, while $M_{i}^{+}$ is its upper bound.
  In our case, we use the following bounds to the control inputs:


  \item Path constraints:
  $$\forall t \quad t_{0} \leq t \leq t_{f}, \quad \quad c(x(t), u(t), t) \leq 0,$$
\end{itemize}
